...
# def getidxset_rand(X, numbase):
#     num_feat = np.shape(X)[1]
#     featset = np.arange(num_feat)
#     np.random.shuffle(featset)
#     idxset = []
#     begin = 0
#     for i in range(numbase):
#         last = min(num_feat, int(num_feat*(i+1)/numbase))
#         idxset.append(featset[begin:last])
#         begin = last
#     return idxset
def getidxset_rand(X, numbase):
    num_feat = np.shape(X)[1]
